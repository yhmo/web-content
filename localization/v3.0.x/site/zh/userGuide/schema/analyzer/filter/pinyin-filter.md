...
<p>The <code translate="no">pinyin</code> filter is typically used with the <a href="/docs/zh/jieba-tokenizer.md">Jieba</a> tokenizer for Chinese text. It works in a custom analyzer filter pipeline and can emit multiple Pinyin token forms for the same Chinese token.</p>
